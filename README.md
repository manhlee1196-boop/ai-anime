...
2. Khi Colab in URL `https://….gradio.live` ở ô cuối, mở URL đó; **không cần tài khoản hoặc mật khẩu**. Giao diện có text-to-image, img2img, tô hoặc tải mask PNG để inpaint; chọn **Anime chuẩn**, **Bán thực 2.5D** hoặc **Tùy chỉnh**, rồi chỉnh prompt/negative, kích thước, steps, CFG, seed, strength, số ảnh và cường độ LoRA đã nạp.
...
**Phong cách và ngón tay/ngón chân:** hai preset chỉ thêm từ khóa minh họa/anime hoặc bán thực 2.5D vào prompt/negative lúc tạo; **không** thay checkpoint WAI v17 hay tải model khác. `Tùy chỉnh` không thêm từ khóa phong cách. Negative mặc định nhắm lỗi ngón *thừa/thiếu/dính/dị dạng*, không liệt kê tay hoặc bàn chân bình thường để loại chúng khỏi ảnh. Đây chỉ là gợi ý, **không bảo đảm** mọi ảnh đúng ngón; nếu còn lỗi hãy khoanh vùng nhỏ bằng tab **Sửa vùng ảnh**. Prompt và negative thực tế chỉ được nhúng vào PNG khi bạn tự bật metadata (tắt mặc định).
