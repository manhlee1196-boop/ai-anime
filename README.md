...
**[WAI_Illustrious_Studio_Colab.ipynb](WAI_Illustrious_Studio_Colab.ipynb)** dành cho người dùng Colab cá nhân: notebook kiểm tra checkpoint WAI-illustrious v17 và LoRA theo SHA-256 trước khi nạp lên GPU Colab, sau đó Gradio tạo **liên kết giao diện tạm thời không cần đăng nhập**. Không cần Cloudflare, Node.js, server GPU khác hay nhập token Cloudflare. Đây là lựa chọn nên dùng nếu bạn chỉ muốn tự tạo ảnh. **[Báo cáo kiểm tra nguồn và vận hành](VERIFICATION.md)** ghi rõ điều đã xác minh và điều còn cần thử trên GPU Colab thật.
...
**Nếu ô 4 dừng lâu ở “Sao chép model từ Drive sang /content”:** checkpoint ~6,94 GB **đã có trên Drive**; ô này tạo bản cache trong `/content` để nạp nhanh hơn, rồi kiểm SHA-256. Notebook mới báo tiến độ sao chép và băm file mỗi ~15 giây. Nếu sao chép quá chậm và bạn muốn bỏ qua, chọn **Runtime → Interrupt execution** (không Restart runtime), đổi `CACHE_MODEL_LOCAL=False` ở ô 3 rồi chạy lại từ ô 3 đến ô 8. Ô 4 vẫn phải **đọc toàn bộ file từ Drive** để kiểm SHA-256 và nạp trực tiếp từ Drive có thể chậm hơn; không tắt xác minh và không xóa file gốc. Nếu bản sao sắp xong, chờ hoàn tất thường lợi hơn.
