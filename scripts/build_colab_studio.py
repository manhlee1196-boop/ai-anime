...
"""
    ...
    introduction = """# WAI Studio · tạo ảnh anime từ Google Colab bằng liên kết riêng
...
2. Ở ô cuối, **tự nhập mật khẩu ít nhất 16 ký tự** trong hộp nhập kín. Notebook không ghi hay in mật khẩu vào output. Chờ dòng `Running on public URL` rồi mở liên kết `https://....gradio.live`; đăng nhập tên **owner** và mật khẩu vừa đặt. Dùng giao diện để tạo ảnh, ảnh → ảnh, tô mask sửa tay/chân/mắt và tải PNG.
...
**Quyền riêng tư:** `share=True` tạo một URL *truy cập được từ Internet* qua proxy của Gradio; mật khẩu chỉ chặn người không có quyền đăng nhập, **không biến URL thành mạng nội bộ**. Không chia sẻ URL/mật khẩu, không nhập mật khẩu vào prompt/ô cấu hình hoặc lưu notebook có output liên kết vào nơi công khai. Gradio nhận dữ liệu qua đường hầm TLS; CPU/GPU và checkpoint vẫn chạy trên Colab. [Giải thích share link](https://www.gradio.app/guides/understanding-gradio-share-links). Ảnh có thể chứa thông tin trong metadata nếu bạn tự bật tùy chọn này.
...
print("Đã chuẩn bị WAI Studio. Ô 8 sẽ tạo liên kết giao diện có mật khẩu.")
"""
    )
    launch = """# @title 8. Mở liên kết giao diện tạm thời (bắt buộc có mật khẩu)
from getpass import getpass
from pathlib import Path

if "studio_runtime" not in globals():
    raise RuntimeError("Chưa nạp model. Chạy các ô 1–7 trước khi tạo link.")
password = getpass("Đặt mật khẩu RIÊNG cho giao diện (>= 16 ký tự; không hiển thị): ")
if len(password) < 16 or password.isspace():
    del password
    raise ValueError("Mật khẩu quá ngắn. Chọn ít nhất 16 ký tự và chạy lại ô này.")
...
print("Tên đăng nhập: owner | Mật khẩu: mật khẩu bạn vừa tự nhập (không hiện trong notebook).")
print("Link sẽ ngừng hoạt động khi Colab dừng/ngắt. KHÔNG chia sẻ link hoặc mật khẩu.")
"""
