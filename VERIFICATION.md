...
| `python -m unittest discover -s tests -v` | **47/47 đạt, 0 bỏ qua**; gồm fake GPU T4 với 2 LoRA, OOM→offload cùng seed, LoRA/hash, bỏ qua cache Drive nhưng vẫn băm SHA, tiến độ sao chép/băm không làm đổi byte, phong cách/18+, ảnh thứ 2 với **PyTorch CPU thật**, các sự kiện Gradio 6 và HTTP nội bộ chặn đường file checkpoint | Model WAI/GPU trong các bài tạo ảnh vẫn được giả lập; test PyTorch chỉ tái hiện lỗi tensor inference-mode |
...
2. Ô 1 phải thấy GPU; ô 2 phải in `✅ Thư viện Studio đã sẵn sàng`; ô 4 in thông báo **xác minh SHA-256 v17**; ô 5 in `Đã xác minh anatomy` / `Đã xác minh eyes` (nếu bật cả hai); ô 6 in `Chế độ sau khi nạp` và VRAM. Nếu ô 4 đang `Sao chép model từ Drive sang /content`, bản mới báo tiến độ khoảng mỗi 15 giây cho bản sao ~6,94 GB và SHA-256. Muốn bỏ qua lần sao chép khi đã có checkpoint trên Drive, **ngắt ô đang chạy**, đổi `CACHE_MODEL_LOCAL=False` ở ô 3 và chạy lại từ ô 3; ô 4 vẫn phải đọc toàn bộ Drive để xác minh hash và nạp model có thể chậm hơn. Nếu một ô lỗi, dừng tại đó, đừng coi ô sau là đã thử thành công.
